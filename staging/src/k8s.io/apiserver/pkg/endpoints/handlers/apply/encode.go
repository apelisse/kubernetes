...
// https://github.com/kubernetes-sigs/structured-merge-diff to the the wire format (api format)
...
func encodePathElementSet(pathElementSet *fieldpath.PathElementSet) (encodedPathElementSet []metav1.FieldPathElement, err error) {
	...
	pathElementList := make(sortablePathElementList, pathElementSet.Size())
	i := 0
	pathElementSet.Iterate(func(pathElement fieldpath.PathElement) {
		pathElementList[i] = &pathElement
		i++
	})
...
func encodeSetNodeMap(setNodeMap *fieldpath.SetNodeMap) (encodedSetNodeMap []metav1.FieldSetNode, err error) {
	// This is the only way to get the number of keys in the setNodeMap,
	// the Size() function will include the size of all the sub paths
	count := 0
	setNodeMap.Iterate(func(_ fieldpath.PathElement) {
		count++
	})

	// Get a list of the keys in the setNodeMap and sort it
	pathElementList := make(sortablePathElementList, count)
	i := 0
...
func encodeValue(v *value.Value) (encodedValue *metav1.FieldValue, err error) {
	...
	if v.FloatValue != nil {
		f := strconv.FormatFloat(float64(*v.FloatValue), 'E', -1, 64)
